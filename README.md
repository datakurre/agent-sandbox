...
**Sandboxed AI coding environment** built on rootless Podman and Nix. Run AI coding agents in an isolated container — `opencode`, `claude`, `copilot`, `codex`, `antigravity`, `pi`, `graph-agent`, or any bundled tool — and opt in only to the integrations you need. All integrations are **disabled by default**.
