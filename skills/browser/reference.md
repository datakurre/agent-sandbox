...
| `connect_over_cdp` to the host's browser refuses/times out | launched without the pasta loopback mapping, so nothing routes to the host's `127.0.0.1`; or under `--proxy`/`--shared-network`, which have no route to the host at all | relaunch with `--podman-args --network=pasta:--map-host-loopback,169.254.1.3 --` and dial that address, see `SKILL.md` |
| `host.containers.internal` refuses even though the host's service is up | that name is podman's `--map-guest-addr`, which resolves to the host's *LAN* address, not its loopback | bind the host service to `0.0.0.0` to use that name, or use the `--map-host-loopback` address for a loopback-bound one |
