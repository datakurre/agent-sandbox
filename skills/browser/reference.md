...
| Video does not play in target tool/player | target environment lacks WebM (VP8) decoder | transcode to MP4 (H.264) — see *Transcoding to MP4 or GIF* in the `screencast` skill's `reference.md` |
| Recording opens with blank seconds, freezes at the end, has no cursor, or the composite is truncated | a recorded context is open only while it is driven, Playwright draws no pointer, and hand-composed ffmpeg is easy to get wrong | see the `screencast` skill — its engine handles all of these, and its `verify` finds them |
