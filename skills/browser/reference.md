...
| `$AGENT_SANDBOX_HOST_PORTS` is unset, or missing the port | launched without `--host-loopback-port 9222`, so nothing reaches the host's `127.0.0.1:9222` | ask the user to run `agent-sandbox browser` and relaunch with the line it prints, see `SKILL.md` |
...
| No `browser_*` MCP tools, but CDP works from a script | relaunched without `-e AGENT_SANDBOX_BROWSER_CDP_PORT=9222` | use both halves of the line `agent-sandbox browser` printed |
| Only one browser reachable when two were started | relaunched with the line the *first* browser printed, which predates the second | each banner reprints the whole command; use the last one, and start every browser before the sandbox |
