...
pub fn run(args: StatusArgs) -> Result<()> {
    ...
    if let Some(ref name) = sandbox_name {
        let valid = name.chars().all(|c| c.is_ascii_alphanumeric() || c == '_' || c == '.' || c == '-');
        let starts_valid = name.chars().next().map(|c| c.is_ascii_alphanumeric()).unwrap_or(false);
    ...
    status_cmd.arg("ps")
        .arg("--filter").arg(format!("name=^{}$", sandbox))
        .arg("--format").arg("{{.Status}}");
    let uptime = String::from_utf8(status_cmd.output()?.stdout)?.trim().to_string();
    row("uptime", &uptime);
    ...
    net_cmd.arg("inspect")
        .arg("--format").arg("{{range $net, $conf := .NetworkSettings.Networks}}{{$net}} {{end}}")
        .arg(&sandbox);
        ...
        if let Ok(policy_dir) = sidecar_mount(&sidecar, "/sidecar_policy") {
            if !policy_dir.is_empty() {
                ...
                if std::path::Path::new(&policy_file).exists() {
                    let rules_cmd = Command::new("grep").arg("-cE").arg("^(allow|deny)_").arg(&policy_file).output()?;
                    ...
                    let allow_cmd = Command::new("grep").arg("-q").arg("^allow_").arg(&policy_file).status()?;
                    if allow_cmd.success() {
                        default = "deny".to_string();
                    }
                    
                    let def_cmd = Command::new("grep").arg("^default ").arg(&policy_file).output()?;
                    ...
                    row("policy", &format!("{} rule(s), default {}        agent-sandbox ctl proxy show", rules, default));
                }
            }
        }
        ...
        if let Ok(log_dir) = sidecar_mount(&sidecar, "/sidecar_shared") {
            if !log_dir.is_empty() {
                ...
                if std::path::Path::new(&log_file).exists() {
                    ...
                    "#;
                    let awk_cmd = Command::new("awk").arg(awk_script).arg(&log_file).output()?;
                    ...
                    if counts.len() == 4 {
                        ...
                        if deny > 0 { summary.push_str(&format!(", {} denied", deny)); }
                        if err > 0 { summary.push_str(&format!(", {} failed", err)); }
                        if live > 0 { summary.push_str(&format!(", {} in flight", live)); }
                        
                        row("network", &format!("{}        agent-sandbox ctl net", summary));
