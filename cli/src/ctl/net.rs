...
pub struct NetArgs {
    #[arg(short, long, help = "stream connections as the proxy records them, until Ctrl-C")]
    pub follow: bool,
...
pub fn run(args: NetArgs) -> Result<()> {
    ...
    if args.follow {
        ...
        let mut tail_child = Command::new("podman")
            .arg("exec").arg(&sidecar).arg("tail").arg("-n").arg("+1").arg("-F").arg("--").arg(log_path)
        ...
        let exists = Command::new("podman").arg("container").arg("exists").arg(&sidecar).status()?;
        if !exists.success() {
            println!("\n{} stopped.", sidecar);
            std::process::exit(0);
        }
        ...
        let cat_out = Command::new("podman").arg("exec").arg(&sidecar).arg("cat").arg(log_path).output();
