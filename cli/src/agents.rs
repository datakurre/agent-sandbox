...
pub fn format_proxy_policy(policy: &ProxyPolicy, source: &str) -> String {
    ...
    for val in &policy.deny_domains {
        lines.push(format!("deny_domains {}", val));
    }
    for val in &policy.secret_domains {
        lines.push(format!("secret_domains {}", val));
    }
    ...
    for val in &policy.deny_ips {
        lines.push(format!("deny_ips {}", val));
    }
...
pub fn format_policy_as_network_toml(cfg: &ProxyConfig) -> String {
    ...
    for d in &cfg.deny_domains {
        advisory.push(format!("deny_domains {}", d));
    }
    for ip in &cfg.deny_ips {
        advisory.push(format!("deny_ips {}", ip));
    }
